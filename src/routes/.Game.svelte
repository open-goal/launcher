<script>
	...
	import { copyDataDirectory, isDataDirectoryUpToDate } from '$lib/utils/data-files';
	...
	<div class="flex-center" in:fade>
		...
		{#if isGameInstalled && !$gameNeedsReinstall}
			{#if !dataDirUpToDate}
				...
				<p>This may overwrite any custom changes to the game's code or textures</p>
				<p>Save files and settings will not be modified</p>
				{#if !updatingDataDir}
					<button class="btn" on:click={syncDataDirectory}> Sync Data Files </button>
				{/if}
				{#if errorText != ''}
					{errorText}
				{/if}
