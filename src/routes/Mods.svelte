<script lang="ts">
  ...
  const filteredMods = $derived.by(() => {
    if (!mods) return {} as AvailableModsByGame;

    const entries =
      game === "all"
        ? Object.entries(mods)
        : [[game, mods[game as SupportedGame] ?? []]];

    return Object.fromEntries(
      entries.map(([gameKey, modsList]) => {
        const search = query.toLowerCase().trim();

        const processed = (modsList as ModInfo[])
          .filter(
            (m: ModInfo) =>
              !search ||
          ...
          .toSorted((a: ModInfo, b: ModInfo): number => {
            switch (sort) {
              case "name":
              ...
              case "release":
                return (
                  Number(
                    Date.parse(
                      b.perGameConfig?.[gameKey as SupportedGame]
                        ?.releaseDate ?? "0",
                    ),
                  ) -
                  Number(
                    Date.parse(
                      a.perGameConfig?.[gameKey as SupportedGame]
                        ?.releaseDate ?? "0",
                    ),
                  )
                );
              case "popularity":
                return Number(b.downloadCount) - Number(a.downloadCount);
              case "updated":
                return (
                  Number(Date.parse(b.versions?.[0]?.publishedDate ?? "0")) -
                  Number(Date.parse(a.versions?.[0]?.publishedDate ?? "0"))
                );
              default:
                return a.displayName.localeCompare(b.displayName);
            }
    ...
    ) as Record<SupportedGame | string, ModInfo[]>;
  });

  const installedMods: (ModInfo & { game: SupportedGame })[] = $derived.by(
    () => {
      if (!filteredMods) return [];

      return Object.entries(filteredMods).flatMap(([gameKey, modsList]) =>
        (modsList as ModInfo[])
          .filter((mod) => mod.installed)
          .map((mod) => ({
            ...mod,
            game: gameKey as SupportedGame,
          })),
      );
    },
  );
  ...
  const isModsEmpty = $derived(() => {
    ...
    return !Object.values(filteredMods).some(
      (modsList) => (modsList as ModInfo[]).length > 0,
    );
  });
      ...
      {#if !isModsEmpty()}
        ...
        {#each Object.entries(filteredMods) as [gameKey, gameMods] (gameKey)}
          <div hidden={gameMods.length === 0} class="py-2">
            ...
            <div id={gameKey} class="grid grid-cols-2 gap-6 mt-2">
              {#each gameMods as mod}
                {#if !mod.installed}
                  <ModCard {mod} activeGame={toSupportedGame(gameKey)!} />
                {/if}
              {/each}
            </div>
          </div>
